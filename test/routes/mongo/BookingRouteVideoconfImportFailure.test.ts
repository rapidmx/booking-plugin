...
// `import("@rapidmx/videoconf-plugin/mongo")` genuinely fails (an incompatible version installed, a corrupted
...
vi.mock("@rapidmx/videoconf-plugin/mongo", () => {
...
describe("Route:BookingMongo Tests (videoconf dynamic-import failure)", () => {
    const logger = Logger();
    const objectFactory: ObjectFactory = new ObjectFactory(config, logger);
    // A minimal fixture WITHOUT `@rapidmx/videoconf-plugin`'s own models registered - see
    // `test/server-mongo-import-failure/models/index.ts`'s doc comment for why this can't reuse `test/server-mongo`.
