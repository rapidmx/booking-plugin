...
describe("_slotPaging", () => {
    ...
    it("asks for one chunk, and continues from its end while the window lasts", async () => {
        ...
        const page = await fetchSlotPage("jane@example.com", "intro-call", "mt-1", initialSlotCursor(90, NOW));
    ...
    it("ends once a chunk reaches the horizon", async () => {
        ...
        const page = await fetchSlotPage("jane@example.com", "intro-call", "mt-1", initialSlotCursor(30, NOW));
    ...
    it("skips a fully booked chunk until it finds a slot", async () => {
        ...
        const page = await fetchSlotPage("jane@example.com", "intro-call", "mt-1", initialSlotCursor(120, NOW));
    ...
    it("stops after MAX_REQUESTS_PER_PAGE empty chunks, returning an empty page that can continue", async () => {
        ...
        const first = await fetchSlotPage("jane@example.com", "intro-call", "mt-1", initialSlotCursor(365, NOW));
        expect(requested).toHaveLength(2);
        expect(first).toEqual({ slots: [], next: { from: NOW + 60 * DAY, horizon: NOW + 365 * DAY } });

        const second = await fetchSlotPage("jane@example.com", "intro-call", "mt-1", first.next!);
    ...
    it("returns an empty, final page when nothing is open in the rest of the window", async () => {
        ...
        const page = await fetchSlotPage("jane@example.com", "intro-call", "mt-1", initialSlotCursor(60, NOW));
        expect(requested).toHaveLength(2);
    ...
    it("resumes a cut-off response from just after its last slot on the next call", async () => {
        ...
        const first = await fetchSlotPage("jane@example.com", "intro-call", "mt-1", initialSlotCursor(30, NOW));
        const second = await fetchSlotPage("jane@example.com", "intro-call", "mt-1", first.next!);
    ...
    it("ends a cut-off response's paging when resuming would start past the booking window", async () => {
        ...
        const page = await fetchSlotPage("jane@example.com", "intro-call", "mt-1", initialSlotCursor(1, NOW));
    ...
    it("never requests past bookingWindowDays when every chunk is fully booked", async () => {
        const requested = mockSlots([[], [], [], []]);
        const page = await fetchSlotPage("jane@example.com", "intro-call", "mt-1", initialSlotCursor(45, NOW));
    ...
    it("returns an empty, final page without fetching for a cursor already at its horizon", async () => {
        ...
        const page = await fetchSlotPage("jane@example.com", "intro-call", "mt-1", { from: NOW + 30 * DAY, horizon: NOW + 30 * DAY });
